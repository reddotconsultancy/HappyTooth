      ...
      <div className="lg:col-span-8 space-y-12">
       {posts.map((post, idx) => (
        <ScrollReveal key={idx} animation="fade-up" delay={idx + 1}>
         <article
          className="bg-white rounded-[2rem] overflow-hidden shadow-md hover:shadow-2xl hover:shadow-navy-blue/5 border border-gray-150 flex flex-col group transition-all duration-500 transform hover:-translate-y-1"
         >
          <div className="relative h-64 sm:h-[22rem] w-full overflow-hidden p-3 bg-gray-50/50">
           <div className="relative w-full h-full rounded-2xl overflow-hidden img-zoom">
            ...
            />
            {/* Floating Date Badge */}
            <div className="absolute top-4 left-4 bg-primary-teal text-white py-2.5 px-4 rounded-2xl shadow-md text-center flex flex-col z-10">
        ...
        <div className="bg-bg-light-blue/40 border border-gray-150 rounded-3xl p-6">
         ...
         <div className="relative">
          <input
           ...
           className="w-full text-xs py-3.5 px-4 bg-white border border-gray-250 focus:border-primary-teal focus:ring-2 focus:ring-primary-teal/15 rounded-xl outline-none transition"
          />
         ...
         <ul className="space-y-4">
          {posts.map((post, idx) => (
           <li key={idx} className="flex gap-3 border-b border-gray-200/40 pb-3 last:border-b-0 last:pb-0">
            <div className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 border border-gray-150 bg-white hover-lift">
             <div className="relative w-full h-full rounded-lg overflow-hidden img-zoom">
              <Image
               src={post.image}
               alt={post.title}
               fill
